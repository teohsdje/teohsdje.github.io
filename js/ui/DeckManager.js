class DeckManager {
    ...
    renderDeckSlots() {
        ...
        slotsContainer.innerHTML = '<h3>Twoja aktualna talia (4 karty):</h3>';
            ...
            if (i < this.deck.length) {
                ...
                slot.innerHTML = `
                    <div class="deck-card">
                        ...
                        <div class="card-cost">Koszt: ${card.cost}</div>
                        <div class="card-upgrade-info">${card.upgradePoints} / ${5 + card.level * 5}</div>
                        <button class="remove-from-deck-btn" data-card-id="${card.id}">Usuń</button>
                    </div>
                `;
                ...
                slot.innerHTML = '<div class="empty-slot">+ Dodaj kartę</div>';
                slot.addEventListener('click', () => {
                    document.querySelector('.available-cards-section').scrollIntoView({ behavior: 'smooth' });
                });
            }
    ...
    renderAvailableCards() {
        ...
        container.innerHTML = '<h3 class="available-cards-section">Dostępne karty do wyboru:</h3>';
        
        this.allCards.forEach(card => {
            const cardElement = document.createElement('div');
            cardElement.className = 'available-card';
